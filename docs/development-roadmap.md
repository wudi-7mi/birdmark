...
已有 `packages/birdmark_ml/birdcut.py`：
...
已有 `packages/birdmark_ml/bird_recognition.py`：
...
- `scripts/download_tpdc_birds.py`
...
两个服务都可以继续使用 `FastAPI`。当前 `apps/inference/app/main.py` 更接近“AI 推理服务 + Web 演示页”的混合体，后续产品化时应逐步拆出业务 API 服务。
