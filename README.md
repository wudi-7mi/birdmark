...
支持 Windows、macOS 和 Linux。建议使用 Python 3.10+，并在本地虚拟环境中运行。
...
macOS/Linux 上的 AI 服务启动方式以 `birdmark-ai` 仓库为准。
...
```bash
export BIRDMARK_INFERENCE_URL="http://127.0.0.1:8000"
```
...
./.venv/bin/python -m uvicorn apps.api.app.main:app --host 127.0.0.1 --port 8100
```
...
启动脚本和 Python 配置都会读取仓库根目录下的 `.env`。系统环境变量优先级更高，适合 systemd、Docker、云平台或 CI/CD 注入。

常用变量：

- `BIRDMARK_API_HOST`：业务 API 监听地址，默认 `127.0.0.1`。容器或局域网部署常用 `0.0.0.0`。
- `BIRDMARK_API_PORT`：业务 API 端口，默认 `8100`。
- `BIRDMARK_INFERENCE_URL`：外部 AI 推理服务地址，默认 `http://127.0.0.1:8000`。
- `BIRDMARK_STORAGE_ROOT`：上传图片、缩略图和 crop 的存储目录，默认 `storage`。
- `BIRDMARK_DATABASE_PATH`：SQLite 数据库路径，默认 `storage/birdmark.sqlite3`。
- `BIRDMARK_WEB_DIR`：Web 静态页面目录，默认 `apps/web`。

生产或长期部署时，建议把 `BIRDMARK_STORAGE_ROOT` 和 `BIRDMARK_DATABASE_PATH` 指向仓库外的持久化目录，例如 `/var/lib/birdmark/storage`，避免代码发布时影响用户上传数据。
...
```bash
export BIRDMARK_API_HOST=0.0.0.0
export BIRDMARK_STORAGE_ROOT=/var/lib/birdmark/storage
export BIRDMARK_DATABASE_PATH=/var/lib/birdmark/storage/birdmark.sqlite3
./start-api.sh
```
